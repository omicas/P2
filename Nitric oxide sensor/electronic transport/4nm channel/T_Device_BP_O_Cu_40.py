...
for index_vg, vg in enumerate(VG):
  transmission_spectrum=nlread('Device_BP_O_Cu_40_%s.nc'%vg, TransmissionSpectrum)[0]
...
#  plt.plot(T,G[index_vg,:], linestyle=linestyles[index_vg],linewidth=2, label='V_G = %s V'%vg)
  plt.semilogy(T,G[index_vg,:], linestyle=linestyles[index_vg],linewidth=2, label='V_G = %s V'%vg)

numpy.savetxt('Device_BP_O_Cu_40_G.txt',G)
