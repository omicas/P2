...
for index_vg, vg in enumerate(VG):
  bias=[]
  current=[]
  iv=nlread('Device_17-10AGNR-PBA-1UDP_60_%s.nc'%vg, IVCurve)[1]
  ...
  #bias=iv.biases()
  #current=iv.currents()
  #numpy.savetxt('Device_17-10AGNR-PBA-1UDP_60_%s_IV.txt'%vg,(bias,current))
  iv=nlread('Device_17-10AGNR-PBA-1UDP_60_%s.nc'%vg, IVCurve)[0]
  biases=numpy.ravel(iv.biases())
  currents=numpy.ravel(iv.currents())
  for i,b in enumerate(biases):
...
numpy.savetxt('Device_17-10AGNR-PBA-1UDP_60_dIdV.txt',(VG,dI))
